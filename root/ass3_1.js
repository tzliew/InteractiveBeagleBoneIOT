...
app.get('/',function(req,res){
       res.sendFile(__dirname + '/a3index.html');
});

var b = require('bonescript');
var led = "P8_13";
var state = 0;
var motion_info = [0,0,0,0,0];     //motion_info format is [Motions Detected, Long Motions, Short Motions, intruders]
var motion_record = [];
var accumulate = 0;		//the numbers of the motions when detected
var printmotion = 1;
b.pinMode(led, 'out');
...
io.sockets.on('connection',function(socket){
       ...
       function printStatus(x) {
                if(x.value === 0){
                        b.digitalWrite(led, 0);
                        console.log("No Motion Detected, Timestamp: " + new Date());
			//push "Empty" to the record
			motion_record.push("E");
                if(accumulate >= 3){
                        motion_info[1] += 1;
                        motion_info[0] += 1;
